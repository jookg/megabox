...
!function( $ ) {
	...
	Datepicker.prototype = {
		...
		fillDow: function(){
			...
			while (dowCnt < this.weekStart + 7) {
				html += '<th scope="col" class="dow">'+DPGlobal.dates.daysMin[(dowCnt++)%7]+'</th>';
			}
			html += '</tr>';
				...
				if (typeof this.markingDates !== 'undefined') {
					for (var i=0; i < this.markingDates.length; i++) {
						...
						if (prevM === (markingDate.getMonth()) && prevMonth.getDate() === markingDate.getDate()) {
							if(prevMonth.getTime() == markingDate.getTime()) {
								clsName += ' c_mint';
							}
							/*
							 * 2015.04.02 
							 * 선택된 날짜의 경우일 때는 '선택됨' 아닐 경우는 '선택 가능일로 표시
							 */
							if (prevMonth.valueOf() === currentDate) {
								aTitle = '선택됨';
								clsName += ' active';
	...
	var DPGlobal = {
		...
		headTemplate: '<thead>'+
							'<tr>'+
								'<th scope="col" class="prev"><a href="javascript:void(0);" class="fa fa-chevron-left a_block" title="이전달 보기"><span class="blind">이전달</span></a></th>'+
								'<th scope="col" colspan="5"></th>'+
								'<th scope="col" class="next"><a href="javascript:void(0);" class="fa fa-chevron-right a_block" title="다음달 보기"><span class="blind">다음달</span></a></th>'+
							'</tr>'+
						'</thead>',
	...
	DPGlobal.template = '<div class="datepicker dropdown-menu" tabIndex="0">'+
							'<div class="datepicker-days">'+
								...
								'<caption class="blind2">날짜선택 달력:각 날짜별로 상영 시간표를 확인하실 수 있습니다.</caption>' +
